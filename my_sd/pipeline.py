...
def generate(prompt: str, 
    ...
    with torch.no_grad():
        ...
        if do_cfg:
            ...
            # (Batch_Size, Seq_Len) -> (Batch_Size, Seq_Len, Dim)
            uncond_contex = clip(uncond_tokens)
            
            # (2, Seq_len, Dim) = (2, 77, 768)
            context = torch.cat([cond_context, uncond_contex])
        else:
        ...
        if input_image:
            ...
            # (Height, Width, Channels) -> (Batch_Size, Height, Width, Channel)
            ...
            # start with random noise, if we're doing text-to-image, strat with random noise N(0,1)
...
def get_time_embedding(timestep):
    ...
    # (1, 160)
    x = torch.tensor([timestep], dytpe=torch.float32)[:, None] * freqs[None]
    # (1, 320)
